<script lang="ts">
	...
	// Parent layout owns the pending badge; we read it for the header line.
	const pendingCount = $derived(data.pendingReports);
	...
	async function handleDismiss(report: ReportResponse) {
		...
		try {
			...
			await invalidate('admin:dashboard');
		} catch (e) {
...
<div class="max-w-4xl mx-auto px-6 pt-6 pb-16">
	<div class="flex items-center justify-between mb-4">
		...
		<select
			...
			onchange={(e) =>
				changeStatusFilter(
					(e.currentTarget as HTMLSelectElement).value as 'pending' | 'dismissed' | 'actioned'
				)}
		...
		{#each reports as report (report.id)}
			<div class="bg-bg-surface border border-border rounded-md p-5 mb-4">
				<div class="flex items-center flex-wrap gap-2 mb-2 text-xs text-text-muted">
					<span
						class="inline-flex items-center gap-1 px-2 py-0.5 rounded font-semibold text-xs bg-danger/15 text-danger"
						>Reported</span
					>
					<span>by</span>
					<a
					...
					{#if report.report_count > 1}
						<span
							>and {report.report_count - 1} {report.report_count - 1 === 1
								? 'other'
								: 'others'}</span
						>
					{/if}
				...
				<div class="text-xs text-text-muted mb-3">
					Reason: <span class="text-text-secondary"
				...
				{#if statusFilter === 'pending'}
					{#if removeTargetReportId === report.id}
						<div
							class="mt-3 bg-bg border border-border rounded-md p-4"
							transition:slide={{ duration: 150 }}
						>
							<div class="text-xs font-semibold text-text-secondary mb-2">
								Remove post — reason (public)
							</div>
							<input
								type="text"
					...
					{:else if suspendTargetReportId === report.id}
						...
						>
							<div class="text-xs font-semibold text-text-secondary mb-2">
								Suspend {report.post_author_name} — reason (public)
							</div>
							<input
								type="text"
					...
					{:else if banTargetReportId === report.id}
						...
						>
							<div class="text-xs font-semibold text-text-secondary mb-2">
								Ban {report.post_author_name} — reason (public)
							</div>
							<input
								type="text"
							...
							/>
							<div class="mb-2">
								<Checkbox bind:checked={banTree}
									>Also ban all users in their downstream invite tree</Checkbox
								>
							</div>
							{#if banError}
