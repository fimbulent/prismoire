...
// (hub dampening, decay, distrust handling) is exercised here too.
...
/// Hub dampening propagates into the closure: traversal *through* a
/// node with in-degree well above `HUB_DAMPEN_THRESHOLD` (5000) has
/// its per-hop decay attenuated, pushing path tails below the
/// visibility threshold.
///
/// Path under test (max_depth = 3): A → H → X → Y, where H has 6501
/// inbound trust edges (the 6500 dummies plus A itself).
///
/// Per-node scores:
/// - H: seeded at 1.0 — direct trust into a hub stays full strength.
/// - X: `1.0 × 0.7 × hub_dampening_factor(6501) ≈ 0.7 × 0.911 ≈ 0.638` —
///   clears 0.45.
/// - Y: `0.638 × 0.7 × 1.0 ≈ 0.447` — X is not a hub, so no
///   additional dampening; this lands just under 0.45 and is
///   pruned by the threshold filter inside `forward_visible_closure`.
///
/// Compare to a non-hub 3-hop chain A → B → X → Y, which scores
/// 1.0 × 0.7 × 0.7 = 0.49 and would clear — the difference here is
/// purely the dampening contribution from the hub on the path.
#[test]
fn forward_visible_closure_excludes_hub_bridged_celebrity() {
    let h = Uuid::from_u128(0x100_0000);
    let x = Uuid::from_u128(0x100_0001);
    let y = Uuid::from_u128(0x100_0002);

    let mut edges: Vec<(Uuid, Uuid)> = Vec::with_capacity(6504);
    // Dummy users 1..=6500 all trust H, pushing H's forward
    // in-degree well above HUB_DAMPEN_THRESHOLD.
    for i in 1u128..=6500 {
        edges.push((Uuid::from_u128(0x200_0000 + i), h));
    }
    edges.push((A, h));
    edges.push((h, x));
    edges.push((x, y));
    ...
    assert!(
        closure.contains(&h),
        "direct trust into hub stays full strength"
    );
    assert!(
        closure.contains(&x),
        "hop past hub (dampened ~0.63) still clears threshold",
    );
    assert!(
        !closure.contains(&y),
        "second hop past hub (~0.44) falls below threshold and must be pruned",
    );
}
