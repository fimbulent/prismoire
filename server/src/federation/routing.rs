...
///    target (trustee `to_key`) the edge points at, NOT the signer;
///    for user-status it is the
...
pub enum ForwardingClass {
    ...
    /// `expansion_filter` (§7.4); key is `target(edge)` (the trustee
    /// `to_key`), NOT the signer.
    TrustEdge,
