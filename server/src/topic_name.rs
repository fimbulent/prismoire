/// Topic name validation and slug generation.
///
...
pub fn validate_topic_name(raw: &str) -> Result<String, String> {
    validate_name(raw, &RULES)
}

/// Compute the URL slug for a topic name.
///
/// Lowercases all characters, replaces spaces and hyphens with underscores.
/// Two topics with identical slugs are considered duplicates.
...
mod tests {
    ...
    fn valid_simple_names() {
        assert_eq!(validate_topic_name("Tech News").unwrap(), "Tech News");
        assert_eq!(validate_topic_name("gaming").unwrap(), "gaming");
        assert_eq!(validate_topic_name("Board Games").unwrap(), "Board Games");
    }

    #[test]
    fn valid_with_hyphens() {
        assert_eq!(
            validate_topic_name("Sci-Fi Movies").unwrap(),
            "Sci-Fi Movies"
        );
    }
    ...
    #[test]
    fn valid_unicode() {
        assert_eq!(validate_topic_name("café culture").unwrap(), "café culture");
        assert_eq!(validate_topic_name("日本語").unwrap(), "日本語");
    }
    ...
    fn rejects_underscores() {
        assert!(validate_topic_name("tech_news").is_err());
    }
    ...
    fn rejects_empty_and_short() {
        ...
        assert!(validate_topic_name("   ").is_err());
        assert!(validate_topic_name("ab").is_err());
        assert!(validate_topic_name("abc").is_ok());
    }
    ...
    fn rejects_too_long() {
        ...
        assert!(validate_topic_name(&"a".repeat(30)).is_ok());
    }

    #[test]
    fn requires_at_least_one_alpha() {
        assert!(validate_topic_name("123").is_err());
        assert!(validate_topic_name("1 2 3").is_err());
        assert!(validate_topic_name("1a2").is_ok());
    }

    #[test]
    fn rejects_leading_trailing_hyphen() {
        assert!(validate_topic_name("-Tech").is_err());
        assert!(validate_topic_name("Tech-").is_err());
        assert!(validate_topic_name("- Tech News").is_err());
        assert!(validate_topic_name("Tech News -").is_err());
    }
    ...
    #[test]
    fn rejects_emoji() {
        assert!(validate_topic_name("Gaming 🎮").is_err());
    }

    #[test]
    fn rejects_punctuation() {
        assert!(validate_topic_name("Tech & Science").is_err());
        assert!(validate_topic_name("Q&A").is_err());
        assert!(validate_topic_name("Hello!").is_err());
    }

    #[test]
    fn rejects_mixed_script() {
        assert!(validate_topic_name("Tеch News").is_err()); // Cyrillic 'е'
    }

    #[test]
    fn slug_basic() {
        assert_eq!(topic_slug("Tech News"), "tech_news");
        assert_eq!(topic_slug("Board Games"), "board_games");
        assert_eq!(topic_slug("gaming"), "gaming");
    }

    #[test]
    fn slug_with_hyphens() {
        assert_eq!(topic_slug("Sci-Fi Movies"), "sci_fi_movies");
    }

    #[test]
    fn slug_case_insensitive() {
        assert_eq!(topic_slug("TECH NEWS"), topic_slug("tech news"));
    }
    ...
    #[test]
    fn slug_unicode() {
        assert_eq!(topic_slug("café culture"), "café_culture");
    }
    ...
    fn nfc_normalization() {
        ...
        assert_eq!(
            validate_topic_name(decomposed).unwrap(),
            validate_topic_name(composed).unwrap()
        );
    }
}
