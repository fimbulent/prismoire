-- Phase 9.5 of federation: extend `users.signup_method` to allow
-- `'federated'`, the provenance tag for stub rows hydrated from
-- received signed objects when a remote-author profile-rev arrives
-- (docs/federation-protocol.md §10.1; docs/federation-impl-plan.md
-- Phase 9.5).
--
-- Phase 7's `20260519165779_relax_signup_method_for_cross_instance.sql`
-- introduced `'cross_instance_register'`; this migration mirrors that
-- one's FK-chain rebuild and additionally folds in the
-- `attachment_blobs.accessed_at` column added in
-- `20260519165782_add_accessed_at_to_attachment_blobs.sql` (§11.5
...
--
-- Why `'federated'` rather than reusing `'cross_instance_register'`
--
--   * Stubs created by passive receipt of remote signed objects have
--     never authenticated to this instance — they have no passkey, no
--     signing key, no session. `'cross_instance_register'` is the
--     §13 ceremony tag for users who actively *moved* their identity
--     here. Keeping the provenance distinct preserves operator
--     visibility (e.g. for export/erasure audit) and gives Phase 9.7
--     a clean upgrade-in-place predicate (`signup_method =
--     'federated'` → promote to `'cross_instance_register'`).
--
...
-- The only change vs. the 165779 `users` definition is the expanded
-- `signup_method` CHECK clause: `'federated'` joins the allowed set.
CREATE TABLE users (
...
-- attachment_blobs includes `accessed_at` (added in 165782).
...
-- Indexes on `users` — mirrors 165779 verbatim.
...
-- §11.5 sloppy-LRU partial index, added in 165782.
...
-- Latest-edge-per-pair view — mirrors 165779 verbatim.
...
-- FTS triggers — mirror 165779 verbatim.
