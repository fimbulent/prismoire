-- Extend the `admin_log.action` CHECK constraint to accept `edit_config`
-- so the admin Config tab handlers (PATCH /api/admin/config) can write a
-- log entry recording who changed which runtime config field and when.
--
-- SQLite can't alter a CHECK constraint in place, so the table is
-- rebuilt. `ban_trust_snapshots` has an FK to `admin_log.id`; both are
-- rebuilt together (leaves-to-root drop, root-to-leaves recreate) per
-- the migrations/CLAUDE.md rebuild-chain pattern.
